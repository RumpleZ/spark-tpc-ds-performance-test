...
SPARK_HOME=$1
OUTPUT_DIR=$2
EXTRA_OPTIONS="--driver-java-options -Dlog4j.configuration=file:///${OUTPUT_DIR}/log4j.properties --conf spark.executor.extraJavaOptions=-Dlog4j.configuration=file:///${OUTPUT_DIR}/log4j.properties --conf spark.sql.crossJoin.enabled=true"
...
do
  num=`printf "%02d\n" $i`
  bin/spark-sql ${SPARK_OPTIONS} ${EXTRA_OPTIONS} -database TPCDS -f ${OUTPUT_DIR}/query${num}.sql > ${OUTPUT_DIR}/query${num}.res 2>&1 
